  ...
  def index
    ...
    if params[:search].present? || params[:distance].present?
      if params[:distance].present? && @current_user && @current_user.latitude && @current_user.longitude
        ...
        @product_infos = ProductInfo.search conditions: {title_body: params[:search]}, :geo => [@current_user.latitude * RATE, @current_user.longitude * RATE], :with => {:geodist => 0.0..distance, price: price_min..price_max}, order: order, page: params[:page], per_page: per_page
      else
